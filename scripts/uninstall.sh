...
remove_template_hooks() {
    ...
    for hook in local-override-post-checkout local-override-pre-commit local-override-post-commit local-override-lib.sh local-override-filter-smudge local-override-filter-clean; do
...
uninstall_filters() {
    ...
    if [[ -z "$repo_root" ]]; then
        info "Not in a git repository, skipping local filter cleanup"
        return 0
    fi
    ...
    for filter_script in local-override-filter-smudge local-override-filter-clean; do
        local filter_file="$hooks_dir/$filter_script"
        if [[ -f "$filter_file" ]]; then
            rm "$filter_file"
            success "Removed: $filter_file"
        fi
    done
