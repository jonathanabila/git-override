...
- **Stale documentation version examples**: documentation version examples (README, CLI `help`, `.pre-commit-hooks.yaml`, `SECURITY.md`) now reference the current release `v0.6.0` instead of the stale `v0.3.0`/`v0.4.0` pins, so copied install/pre-commit snippets fetch the current build
