...
- `managed_hook_types()` / `managed_filter_scripts()` / `managed_runtime_files()` - The managed-runtime manifest (in the shared resolver): the single definition of what the installers materialize and the uninstaller may remove; install.sh, uninstall.sh, the test fixtures, and the bench all iterate these
- `install_managed_runtime_from_checkout()` - Copy-based materializer (in the shared resolver) for source checkouts: manifest-driven copy of the runtime (and optionally the entry hooks under bare git hook names) into a hooks dir; used by `install_test_hooks` and the filter bench (install.sh keeps its own acquisition — it can run via curl with no checkout)
...
- The decisions live in the shared resolver as directly testable functions — `precommit_plan_restores` (grouped-restore plan), `targets_for_override_in_entries` (the Group expansion), `staged_blob_matches_override` (byte-exact leak predicate), `precommit_find_merge_leak` (merge/cherry-pick backstop), `precommit_new_target_leaks` (new-target refusal) — the hook keeps only git mutations and messages; the reapply state it leaves for post-commit is written by `record_reapply_state` (in `local-override-lib.sh`, the writer counterpart of `reapply_post_commit_state`)
